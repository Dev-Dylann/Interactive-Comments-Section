...
function ReplyInput({ setIsReplying, replyingComment }: Props) {
    ...
    return (
        <form
            className='p-4 grid grid-cols-2 gap-y-4 bg-white rounded-lg'
            onSubmit={(e) => {
                e.preventDefault()
                setReplyContent('')
            }}
        >
            <textarea
                autoFocus
                className='border border-lightGray rounded-lg px-6 py-2 resize-none text-grayBlue col-span-full focus:outline-modBlue focus:outline-1'
            ...
            <button
                ...
                type="submit"
                className='w-fit place-self-end bg-modBlue text-white font-bold px-4 py-2 rounded-lg reply-button disabled:brightness-75'
                onClick={handleReply}
            >
                REPLY
            </button>
